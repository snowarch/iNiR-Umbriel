...
function setup_desktop_settings(){
  ...
  local setup_compositor_target=""
  setup_compositor_target="$(inir_install_compositor_service 2>/dev/null || true)"
  if [[ "$setup_compositor_target" != "umbriel-session.target" ]]; then
    # Niri exports XCURSOR_* to spawned clients; materialize the same cursor for
    # X11/Electron clients that resolve the core X cursor separately.
...
PORTAL_EOF
    ...
    log_success "Umbriel portal integration is owned by xdg-desktop-portal-umbriel"
  fi
